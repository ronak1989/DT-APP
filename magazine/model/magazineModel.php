...
class magazineModel extends Database {
	...
	private function generateOrder($_insertVals = array()) {
		...
		if (trim($decrypted_string) != '') {
			$this->_modelQuery = 'INSERT INTO `order_details`(`package_id`,`uid`,`subscription_type`,`subscription_amount`,`delivery_method`,`promotional`)VALUES(:package_id,:uid,:subscription_type,:subscription_amount,:delivery_method,"' . $decrypted_string . '")';
		} else {
